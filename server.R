...
shinyServer(
  function(input,output) {
    ...
    output$Season <- renderText({input$season})
    
    output$table <- renderDataTable(maketable(df=spain,Season = input$season,tier=1))
    
    
    
    output$performance_Plot <- renderPlot({top_contests <- comparePerformance(input$season)
    ...
    legend("topright", legend = c("Goals difference Home", "Goals difference Away"), col=c("magenta","blue"), pch = c('H','A'))
